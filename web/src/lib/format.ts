export function formatBytes(value: number | undefined, precision = 1): string {
  ...
  const order = Math.min(Math.floor(Math.log(Math.abs(value)) / Math.log(1024)), units.length - 1)
...
export function isTerminalOperation(value: string): boolean {
  return ['succeeded', 'failed', 'cancelled', 'partially_succeeded'].includes(value)
}

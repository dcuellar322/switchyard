...
export async function loadProjectSnapshot(project: Project): Promise<ProjectSnapshot> {
  ...
  if (runtime.status === 'rejected') warnings.push('Runtime observation unavailable')
  if (git.status === 'rejected') warnings.push('Git observation unavailable')
  if (metrics.status === 'rejected') warnings.push('Resource samples unavailable')
...
export async function loadProjectSnapshots(
  ...
  for (let index = 0; index < projects.length; index += concurrency) {
    snapshots.push(
      ...(await Promise.all(projects.slice(index, index + concurrency).map(loadProjectSnapshot))),
    )
  }
  return snapshots
}

...
import { loadProjects, runRuntimeAction } from '../../projects/api'
...
export function useDashboard() {
  ...
  const runtimeMutation = useMutation({
    mutationFn: ({ projectId, action }: { projectId: string; action: RuntimeAction }) =>
  ...
  const visibleSnapshots = computed(() => {
    ...
    const result = allSnapshots.value.filter((snapshot) => {
      ...
      const matchesStatus =
        ...
        (statusFilter.value === 'running' ? isActiveState(state) : state === statusFilter.value)
      const matchesTag =
        tagFilter.value === 'all' || snapshot.project.tags.includes(tagFilter.value)
    ...
    return result.sort((left, right) => {
      ...
      if (sortOrder.value === 'status')
        return (left.runtime?.state ?? 'unknown').localeCompare(right.runtime?.state ?? 'unknown')
      return (
  ...
  const runningCount = computed(
    () => allSnapshots.value.filter((snapshot) => isActiveState(snapshot.runtime?.state)).length,
  )
  const serviceCount = computed(() =>
    allSnapshots.value.reduce(
      (total, snapshot) =>
        total +
        (snapshot.runtime?.services.filter((service) => service.state === 'running').length ?? 0),
      0,
    ),
  )
  const memoryBytes = computed(() =>
    allSnapshots.value.reduce(
      (total, snapshot) =>
        total +
        (snapshot.metrics?.reduce((subtotal, metric) => subtotal + metric.memoryBytes, 0) ?? 0),
      0,
    ),
  )
  ...
  const partialCount = computed(
    () => allSnapshots.value.filter((snapshot) => snapshot.warnings.length > 0).length,
  )
  ...
  async function runLifecycle(snapshot: ProjectSnapshot, action: RuntimeAction) {
    ...
    try {
      ...
      operationError.value =
        cause instanceof Error ? cause.message : 'The lifecycle operation could not be queued.'
    } finally {

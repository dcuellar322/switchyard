...
<template>
  <article class="ai-card" aria-labelledby="ai-onboarding-title">
    ...
    <p class="explanation">
      Switchyard sends only the redacted evidence shown below. Deterministic facts keep priority,
      and every provider change must cite evidence.
    </p>
    ...
    <ul v-if="providers.some((provider) => !provider.available)" class="provider-notes">
      <li v-for="provider in providers.filter((item) => !item.available)" :key="provider.id">
    ...
    <section v-if="preview" class="receipt" aria-labelledby="evidence-receipt-title">
      <div class="receipt__summary">
        <div>
          <span>Payload</span
          ><strong>{{ preview.bundle.encodedBytes.toLocaleString() }} bytes</strong>
        </div>
        <div>
          <span>Findings</span><strong>{{ preview.bundle.evidence.length }}</strong>
        </div>
        <div>
          <span>Redactions</span><strong>{{ preview.bundle.redactionCount }}</strong>
        </div>
        <div>
          <span>Digest</span><code>{{ preview.sha256.slice(0, 12) }}…</code>
        </div>
      </div>
      ...
      <div class="evidence-list">
        <details v-for="item in preview.bundle.evidence" :key="item.id">
          <summary>
            ...
            ><code>{{ item.sourcePath }}:{{ item.location.startLine }}</code
      ...
      <label class="consent">
        <input
          type="checkbox"
          :checked="consented"
          @change="emit('update:consented', ($event.target as HTMLInputElement).checked)"
        />
        I reviewed this exact redacted payload and authorize sending it to the selected provider.
      </label>
      <div class="ai-actions">
        <button type="button" :disabled="pending || !consented" @click="emit('start')">
          Generate reviewable proposal
        </button>
      </div>
    </section>
  </article>
...
.ai-card {
  ...
  border-radius: 14px;
  background: linear-gradient(145deg, rgba(18, 31, 46, 0.96), rgba(14, 19, 26, 0.96));
}
.ai-card__heading {
  display: flex;

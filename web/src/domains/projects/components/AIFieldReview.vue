...
<template>
  <article v-if="operation || run" class="review" aria-live="polite">
    <div class="review__heading">
      <div>
        <p class="eyebrow">Provider operation</p>
        <h2>Assisted proposal review</h2>
      </div>
      <span class="state" :class="`state--${run?.state ?? operation?.state}`">{{
        run?.state ?? operation?.state
      }}</span>
    </div>
    <p v-if="operation && ['queued', 'running'].includes(operation.state)" class="message">
      The provider is working against the approved evidence bundle. You can cancel without changing
      the deterministic proposal.
    </p>
    <button
      v-if="operation && ['queued', 'running'].includes(operation.state)"
      class="button--danger"
      type="button"
      :disabled="cancelling"
      @click="$emit('cancel')"
    >
      {{ cancelling ? 'Cancelling…' : 'Cancel provider run' }}
    </button>
    <p v-if="run?.errorMessage" class="message message--error" role="alert">
      {{ run.errorMessage }}
    </p>
    ...
    <template v-if="run?.state === 'succeeded'">
      ...
      <p
        v-for="warning in [...run.warnings, ...run.dryRun.warnings]"
        :key="warning"
        class="message"
      >
        {{ warning }}
      </p>
      <p v-for="error in run.dryRun.errors" :key="error" class="message message--error">
        {{ error }}
      </p>
      <section v-if="run.fields.length">
        ...
        <div class="field-table" role="table" aria-label="Assisted field provenance">
          <div
            ...
            :key="`${field.path}-${field.source}`"
            class="field-row"
            role="row"
          >
            <code>{{ field.path }}</code
            ><span :class="`source source--${field.source}`">{{ field.source }}</span
            ><strong>{{ Math.round(field.confidence * 100) }}%</strong>
            <p>{{ field.rationale || field.warnings.join(' ') }}</p>
            <small>{{ field.evidenceIds.join(', ') || 'deterministic rule' }}</small>
          </div>
        </div>
      </section>
      <section v-if="run.conflicts.length">
        ...
        <div v-for="conflict in run.conflicts" :key="conflict.path" class="conflict">
          ...
          ><strong>Kept deterministic value</strong>
          <span
            >Provider proposed <code>{{ JSON.stringify(conflict.proposedValue) }}</code></span
          >
        </div>
      </section>
      <details class="receipt">
        <summary>Provider receipt · {{ run.bundleSha256.slice(0, 12) }}…</summary>
...
.dry-run--ready {
  border-color: rgba(84, 212, 154, 0.35);
}
.dry-run span {
  color: var(--muted);
  font-size: 12px;
...
.field-row {
  display: grid;
  grid-template-columns: 1.2fr auto 55px;
  gap: 10px;
  padding: 11px;
  border: 1px solid var(--border);
  border-radius: 8px;
...
.source {
  ...
  border-radius: 999px;
  color: var(--cyan);
  background: rgba(83, 174, 255, 0.1);
  font-size: 10px;
}
.source--rejected {
  color: var(--red);

...
const activeTab = ref<ProjectTab>(projectTabs.includes(route.query.tab as ProjectTab) ? (route.query.tab as ProjectTab) : "overview");
const {
  project, runtime, health, metrics, git, actions, manifest, environments,
...
<template>
  <section class="project-view" aria-labelledby="project-title">
    <div v-if="project.isPending.value" class="loading" aria-live="polite"><span></span><span></span><span></span></div>
    ...
    <template v-else-if="project.data.value">
      ...
      <div v-if="activeTab === 'overview'" id="panel-overview" role="tabpanel" tabindex="0" aria-labelledby="tab-overview">
        <ProjectOverviewTab
          :project-id="projectId" :runtime="runtime.data.value" :runtime-pending="runtime.isPending.value"
      ...
      <div v-else-if="activeTab === 'logs'" id="panel-logs" class="single-panel" role="tabpanel" tabindex="0" aria-labelledby="tab-logs">
        <ProjectLogsTab :entries="liveLogs" :connection="logConnection.state.value" />
      </div>
      <div v-else-if="activeTab === 'terminal'" id="panel-terminal" class="single-panel" role="tabpanel" tabindex="0" aria-labelledby="tab-terminal">
        <TerminalPanel
      ...
      <div v-else-if="activeTab === 'git'" id="panel-git" class="single-panel" role="tabpanel" tabindex="0" aria-labelledby="tab-git">
        <ProjectGitTab
          :git="git.data.value" :environments="environments.data.value ?? []"
          :environments-pending="environments.isPending.value" :environments-error="environments.isError.value"
          :registration-pending="registerWorktrees.isPending.value" :registration-error="registerWorktrees.error.value?.message"
          @register="registerWorktrees.mutate()"
        />
      </div>
      <div v-else-if="activeTab === 'ports'" id="panel-ports" class="single-panel" role="tabpanel" tabindex="0" aria-labelledby="tab-ports">
        <ProjectPortsTab :ports="projectPorts" />
      </div>
      <div v-else-if="activeTab === 'storage'" id="panel-storage" class="single-panel" role="tabpanel" tabindex="0" aria-labelledby="tab-storage">
        <ProjectStorageTab :project-id="projectId" />
      </div>
      <div v-else-if="activeTab === 'agents'" id="panel-agents" class="single-panel" role="tabpanel" tabindex="0" aria-labelledby="tab-agents">
        <AgentSessionsPanel :project-id="projectId" :environments="environments.data.value ?? []" @terminal="selectTab('terminal')" />
      </div>
      <div v-else id="panel-config" class="single-panel" role="tabpanel" tabindex="0" aria-labelledby="tab-config">
        <ProjectConfigTab :manifest="manifest.data.value" :pending="manifest.isPending.value" :error="manifest.isError.value" />
      </div>
    </template>
  </section>
</template>

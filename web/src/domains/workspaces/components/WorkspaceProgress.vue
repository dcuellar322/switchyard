...
<template>
  <section class="progress-panel" aria-labelledby="workspace-progress-title">
    <header>
      <div>
        ...
        <h2 id="workspace-progress-title">
          {{ execution.kind }} · {{ execution.state.replaceAll('_', ' ') }}
        </h2>
      </div>
      <span :class="`summary summary--${execution.state}`">{{ execution.policy }} policy</span>
    </header>
    <p v-if="execution.errorMessage" class="execution-error" role="status">
    ...
    <footer>
      ...
      <span v-if="execution.removeData" class="data-warning"
        >Runtime data removal was explicitly requested</span
      >
...
.execution-error {
  margin: 0;
  padding: 11px 19px;
  border-bottom: 1px solid rgba(255, 115, 115, 0.22);

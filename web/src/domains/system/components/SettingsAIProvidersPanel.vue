...
<template>
  <article class="settings-panel settings-panel--wide">
    <div class="settings-panel__head">
      <div>
        <p>Provider-neutral assistance</p>
        <h2>AI providers</h2>
      </div>
      <span>Restart</span>
    </div>
    <p class="settings-help">
      Only adapter metadata and credential references are stored. Paste no token here;
      <code>env:NAME</code> resolves from the daemon environment after restart.
    </p>
    <label class="settings-default"
      >Default provider<select v-model="settings.ai.defaultProvider">
        <option value="none">None — deterministic onboarding only</option>
        <option
          v-for="provider in settings.ai.providers"
          :key="provider.id"
          :value="provider.id"
    ...
    <div class="provider-grid">
      <section v-for="provider in settings.ai.providers" :key="provider.id" class="provider-card">
        ...
        <label v-if="provider.id !== 'openai-compatible'"
          >Executable<input
            v-model.trim="provider.executable"
        ...
        /></label>
        <label
          >Model <small>optional</small><input v-model.trim="provider.model" autocomplete="off"
        /></label>
        <label v-if="provider.id === 'openai-compatible'"
          >Credential reference<input
            v-model.trim="provider.credentialReference"
            :required="provider.enabled"
            autocomplete="off"

...
test("scans a repository into an evidence-backed review", async ({ page }) => {
  ...
  await page
    ...
    .fill("./test/fixtures/mixed-project");
  ...
  await expect(
    page.getByRole("button", { name: "Approve and trust project" }),
  ).toBeEnabled();
  await expect(
    page.getByText("switchyard-secret-canary-never-return"),
  ).toHaveCount(0);

  await page.getByRole("button", { name: "Approve and trust project" }).click();
  await expect(page).toHaveURL(/\/projects\//);
  await expect(
    page.getByRole("heading", { name: "Switchyard Mixed Fixture" }),
  ).toBeVisible();
...
test("manages a native process project through durable browser operations", async ({
  page,
}) => {
  ...
  await page
    .getByLabel("Repository path")
    .fill("./test/fixtures/node-single-process");
  await page.getByRole("button", { name: "Scan repository" }).click();
  await expect(
...
test("manages a real Compose project through the same browser workflow", async ({
  ...
  execFileSync(
    "go",
    ["build", "-trimpath", "-o", ".switchyard-fixture-server", "server.go"],
    {
      cwd: fixtureRoot,
      env: {
        ...process.env,
  ...
  const cleanupFixture = () => execFileSync(
    "docker",
    [
      "compose",
      "--project-directory",
  ...
  try {
    ...
    await page
      .getByLabel("Repository path")
      .fill("./test/fixtures/compose-runtime");
    await page.getByRole("button", { name: "Scan repository" }).click();
    ...
    if (await approveButton.isEnabled()) {
      ...
      await page
        .locator(".existing-projects a")
        .filter({ hasText: "compose-runtime" })
        .click();
    }
    await expect(page).toHaveURL(/\/projects\//);
    ...
    await expect
      .poll(
        ...
        { timeout: 90_000, message: "Compose start operation should succeed" },
      )
      .toBe("succeeded");
    await expect
      .poll(

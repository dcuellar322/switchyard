...
test('renders the alpha shell, keyboard palette, and live daemon settings', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Type a command or project' }).fill('scan')
  ...
  await expect(page.getByText(/Restart the daemon to apply/)).toContainText('retention')

...
test("resource intelligence matches the approved hierarchy and attribution states", async ({
  ...
  await page.route("**/api/v1/resources", (route) =>
    route.fulfill({
      ...
      json: {
        observedAt,
        projects,
        storage: { bytes: 6_442_450_944, reclaimableBytes: 1_342_177_280, classification: "shared", resourceCount: 4 },
        footprint: { databaseBytes: 8_388_608, databaseWalBytes: 1_048_576, databaseShmBytes: 32_768, logBytes: 67_108_864, logSegments: 12, metricRows: 4_812, classification: "exclusive" },
        retention: { sampleIntervalSeconds: 10, rawSeconds: 3_600, minuteSeconds: 86_400, quarterHourSeconds: 2_592_000, maximumHistoryPoints: 1_000, logSeconds: 604_800, logBytes: 268_435_456 },
        warnings: [],
      },
    }),
  );
  await page.route("**/api/v1/resources/storage", (route) =>
    route.fulfill({
      ...
      json: {
        connected: true,
        observedAt,
        summary: { bytes: 6_442_450_944, reclaimableBytes: 1_342_177_280, classification: "shared", resourceCount: 4 },
        projects: [],
        resources: [
          { kind: "container", id: "f3e18c5b7d21", name: "alpha-api-1", projectIds: ["alpha"], bytes: 134_217_728, reclaimable: false, classification: "exclusive", reason: "Writable layer is uniquely associated through the canonical Compose project label." },
          { kind: "image", id: "sha256:126d9fe84b0a", name: "alpha-api:dev", projectIds: ["alpha"], bytes: 1_073_741_824, reclaimable: false, classification: "estimated", reason: "Associated with one project, but layer exclusivity is not proven." },
          { kind: "volume", id: "alpha_postgres", name: "alpha_postgres", projectIds: ["alpha"], bytes: 2_147_483_648, reclaimable: true, classification: "exclusive", reason: "Local volume is referenced only by one canonical Compose project." },
          { kind: "build_cache", id: "cache-93a1", name: "regular", projectIds: [], bytes: 536_870_912, reclaimable: true, classification: "shared", reason: "Docker reports this build cache record as shared." },
        ],
  ...
  await page.route("**/api/v1/projects/*/metrics/history**", (route) => {
    ...
    return route.fulfill({
      contentType: "application/json",
      json: { projectId, serviceId: "", resolutionSeconds: 60, from: points[0]!.timestamp, to: observedAt, points },
    });
  });

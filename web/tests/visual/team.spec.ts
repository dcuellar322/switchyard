...
test('signed team configuration keeps trust, policy, and registry provenance visible', async ({
  page,
}) => {
  await installAlphaMocks(page)
  await page.route('**/api/v1/team/publishers', (route) =>
    route.fulfill({
      ...
      json: [
        {
          ...
          publicKey: 'cHVibGlj',
          trustedAt: observedAt,
        },
      ],
    }),
  )
  await page.route('**/api/v1/team/bundles**', (route) =>
    route.fulfill({
      contentType: 'application/json',
      json: [
        {
          ...
          payload: {
            ...
            allowedPluginPublishers: [publisherId],
            telemetryAllowed: false,
          },
          signature: { keyId: publisherId, algorithm: 'Ed25519', value: 'reviewed-signature' },
          installedAt: observedAt,
        },
      ],
    }),
  )
  await page.route('**/api/v1/team/policy', (route) =>
    route.fulfill({
      contentType: 'application/json',
      json: {
  ...
  await page.route('**/api/v1/plugin-registry', (route) =>
    route.fulfill({
      contentType: 'application/json',
      json: [
        {
          ...
          version: '1.2.0',
          summary: 'Reviewed development-container metadata.',
          publisher: publisherId,
          downloadUrl: 'https://plugins.example.test/devcontainer.tar.gz',
          sha256: 'b'.repeat(64),
  ...
  await expect(page).toHaveScreenshot('signed-team-configuration.png', {
    animations: 'disabled',
    fullPage: true,
  })
})

test('telemetry consent displays the exact bounded payload and opt-out controls', async ({
  page,
}) => {
  await installAlphaMocks(page)
  await page.route('**/api/v1/telemetry', (route) =>
    route.fulfill({
      ...
      json: {
        settings: {
          enabled: true,
          endpoint: 'https://metrics.example.test/v1',
          installationId: 'anonymous-preview',
          updatedAt: observedAt,
        },
        counters: [
          { name: 'runtime.operation', value: 8 },
          { name: 'remote.operation', value: 2 },
        ],
        preview: {
  ...
  await expect(page).toHaveScreenshot('telemetry-consent.png', {
    animations: 'disabled',
    fullPage: true,
  })
})

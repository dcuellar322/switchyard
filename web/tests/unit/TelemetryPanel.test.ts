...
test('is off by default and requires review of the exact endpoint', async () => {
  ...
  expect(
    screen.getByText(/never projects, paths, logs, commands, or machine identities/i),
  ).toBeInTheDocument()
  const button = screen.getByRole('button', { name: 'Enable anonymous counters' })
  expect(button).toBeDisabled()
  await fireEvent.update(
    screen.getByLabelText('HTTPS collection endpoint'),
    'https://metrics.example.test/v1',
  )
  await fireEvent.click(screen.getByRole('checkbox', { name: /reviewed the exact payload/i }))
  await fireEvent.click(button)
  await waitFor(() =>
    expect(api.enableTelemetry).toHaveBeenCalledWith('https://metrics.example.test/v1'),
  )

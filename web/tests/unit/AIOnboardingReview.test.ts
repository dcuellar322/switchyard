...
test('shows unavailable providers and the exact redacted evidence before consent', async () => {
    ...
    props: {
      providers: [
        ...
        },
        {
          ...
          supportedBudgetKinds: [],
        },
      ],
      preview: {
        bundle: {
          version: 'switchyard.dev/ai-evidence/v1alpha1',
          projectId: 'project-1',
          proposalId: 'proposal-1',
          deterministicCandidate: {},
          confidenceByField: {},
          unresolved: ['/runtime/driver'],
          evidence: [
            {
              id: 'ev-1',
              kind: 'node.script',
              ...
              confidence: 0.9,
              data: { command: ['pnpm', 'dev'], token: '[REDACTED]' },
              warnings: [],
              excerpt: '"dev": "vite"',
              truncated: false,
            },
        ...
        encoded: { evidence: [{ id: 'ev-1', token: '[REDACTED]' }] },
        sha256: 'a'.repeat(64),
  ...
  expect(
    screen.getByText(
      (_, element) =>
        element?.tagName === 'LI' &&
        element.textContent?.includes('Claude Code: executable not found') === true,
    ),
  ).toBeInTheDocument()
        ...
        fields: [
          {
            ...
            evidenceIds: ['ev-1'],
            rationale: '',
            warnings: ['command not backed by evidence'],
          },
        ],
        conflicts: [
          {
            path: '/metadata/name',
            deterministicValue: 'Safe',
            proposedValue: 'Invented',
            resolution: 'kept_deterministic',
          },
        ],
        warnings: ['provider action was filtered'],
        dryRun: {
          valid: false,
          schemaValid: true,

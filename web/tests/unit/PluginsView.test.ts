...
beforeEach(() => {
  vi.clearAllMocks()
  api.loadPlugins.mockResolvedValue([plugin])
  api.loadPluginLogs.mockResolvedValue([])
  api.approvePlugin.mockResolvedValue({ ...plugin, trust: 'trusted' })
})

function renderView() {
  return render(PluginsView, {
    global: {
      plugins: [
        [
          VueQueryPlugin,
...
test('requires exact identity review and exposes requested scopes before trust', async () => {
  ...
  await waitFor(() =>
    expect(api.approvePlugin).toHaveBeenCalledWith('fixture-inspector', 'a'.repeat(64)),
  )
})

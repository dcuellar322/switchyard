...
vi.mock('../../src/domains/projects/api', () => ({
  ...
  loadProjectActions: vi.fn().mockResolvedValue({
    projectId: 'alpha',
  ...
  loadEffectiveManifest: vi.fn().mockResolvedValue({
...
test('keeps project controls usable and honest when Docker is unavailable', async () => {
  ...
  expect(screen.getByRole('link', { name: 'Inspect project storage →' })).toHaveAttribute(
    'href',

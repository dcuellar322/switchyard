...
const api = vi.hoisted(() => ({
  loadMachineSnapshot: vi.fn(), loadMachines: vi.fn(), refreshMachine: vi.fn(), registerMachine: vi.fn(),
  removeMachine: vi.fn(), runMachineOperation: vi.fn(), saveMachineAccess: vi.fn(),
}))
vi.mock('../../src/domains/fleet/api', () => api)
import FleetView from '../../src/domains/fleet/views/FleetView.vue'

const machine = {
  id: 'machine-1', name: 'Build box', endpoint: 'https://127.0.0.1:19618', certificateFingerprint: 'a'.repeat(64),
  credentialConfigured: true, enabled: true, capabilities: ['inventory.read', 'project.operate'],
  grantedCapabilities: ['inventory.read'], state: 'online', peerId: 'peer-1', peerVersion: '1.0.0', os: 'linux', architecture: 'amd64',
  createdAt: '2026-07-16T12:00:00Z', updatedAt: '2026-07-16T12:00:00Z',
} as const
const snapshot = {
  identity: { protocolVersion: 'switchyard.remote/v1', machineId: 'peer-1', name: 'Build box', version: '1.0.0', os: 'linux', architecture: 'amd64', capabilities: ['inventory.read', 'project.operate'] },
  projects: [{ id: 'project-1', slug: 'example', displayName: 'Example API', runtime: 'compose', state: 'running', health: 'healthy', degraded: false }],
  environments: [], observedAt: '2026-07-16T12:00:00Z',
} as const
...
test('companion is read-only and omits trust and mutation controls', async () => {
  ...
  expect(screen.queryByRole('heading', { name: 'Remote lifecycle operation' })).not.toBeInTheDocument()
})

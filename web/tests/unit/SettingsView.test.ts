...
vi.mock('../../src/domains/system/settingsApi', async () => ({
  ...settingsAPI,
...
vi.mock('../../src/domains/system/api', () => ({
  ...
  loadHostObservation: vi.fn().mockResolvedValue({
    ...
    docker: { connected: true, storageBytes: 0, reclaimableBytes: 0, attribution: 'shared' },
...
beforeEach(() => {
  ...
  settingsAPI.saveDaemonSettings.mockImplementation(async (settings) => ({
    settings: { ...settings, revision: 2, updatedAt: '2026-07-16T12:01:00Z' },
    pendingRestart: ['retention'],
  }))
})

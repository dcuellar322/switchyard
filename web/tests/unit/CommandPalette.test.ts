...
vi.mock("../../src/domains/projects/api", () => ({
  loadProjects: vi
    .fn()
    .mockResolvedValue([
      { id: "alpha", displayName: "Alpha App", slug: "alpha", tags: [] },
    ]),
...
test("filters and executes navigation commands entirely from the keyboard", async () => {
  ...
  expect(
    await screen.findByRole("option", { name: /Show all error logs/ }),
  ).toHaveAttribute("aria-selected", "true");
  await fireEvent.keyDown(document, { key: "Enter" });

  await waitFor(() =>
    expect(router.currentRoute.value.fullPath).toBe("/logs?level=error"),
  );
